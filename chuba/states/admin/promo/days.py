...
class AdminPromoDaysForm(DiscordMessageStateForm):

    @property
    def data(self) -> dict:
        return Chuba.config.get_value("forms", "AdminPromoDays")


class AdminPromoDays(DiscordMessageState):

    """Ввод количества промокодов
    """

    form = AdminPromoDaysForm()

    @message()
    async def handle_promo_days(self, ctx: StateContext):
        event = ctx.event
        ...
        if isinstance(event, MessageEvent):
            ...
            if content.isdigit():
                with ctx.data() as data:
                    data["Days"] = int(content)
                await ctx.set("AdminPromoConfirm")
            else:
                pass
